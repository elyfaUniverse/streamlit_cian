...
from streamlit_folium import st_folium
import folium
from visual import show_model_analysis
...
def load_model_and_bins():
    try:
        ...
        st.error(f"Файл не найден: {e}. Убедитесь, что 'catboost_final_model.pkl' и 'bins.json' в одной папке.")
...
def get_bin(coord, intervals, labels):
    for (left, right), label in zip(intervals, labels):
        if left < coord <= right:
            return label
    return labels[0]

# 2.Списки районов и метро 
districts_list = [
...
# Словарь: муниципальный округ-район
okrug_to_district = {
...
def reverse_geocode(lat, lon):
    ...
    try:
        ...
        if location and location.raw:
            ...
            # Расширенный поиск района: добавил county и другие поля
            district_raw = (
...
if lat_intervals and lng_intervals:
    lat_bin = get_bin(lat, lat_intervals, lat_bin_labels)
    lng_bin = get_bin(lng, lng_intervals, lng_bin_labels)
else:
    lat_bin = '(59.926, 59.957]'
    lng_bin = '(30.265, 30.319]'

metro_bin_default = '(6.0, 9.0]'
...
user_dict = {
    ...
    "lat_bin": lat_bin,
    "lng_bin": lng_bin,
    "metro_bin": metro_bin_default,
}

#Скрытое окно для проверки признаков
with st.expander("Технические признаки", expanded=False):
    st.write("### Все признаки, которые будут отправлены в модель")
    debug_df = pd.DataFrame(list(user_dict.items()), columns=["Признак", "Значение"])
...
full_input = {f: user_dict.get(f, 0) for f in features}
